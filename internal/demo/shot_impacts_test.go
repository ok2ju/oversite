...
func TestPairShotsWithImpacts(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			events: []GameEvent{
				{Tick: 100, Type: "weapon_fire", AttackerSteamID: "A", ExtraData: map[string]interface{}{"yaw": 0.0}},
		...
		},
		{
			...
			events: []GameEvent{
				...
				{Tick: 106, Type: "weapon_fire", AttackerSteamID: "A", ExtraData: map[string]interface{}{"yaw": 0.0}},
		...
		},
		{
			...
			events: []GameEvent{
				{Tick: 100, Type: "weapon_fire", AttackerSteamID: "A", ExtraData: map[string]interface{}{"yaw": 0.0}}, // miss
				{Tick: 106, Type: "weapon_fire", AttackerSteamID: "A", ExtraData: map[string]interface{}{"yaw": 0.0}}, // hits
		...
		},
		{
			...
			events: []GameEvent{
				{Tick: 100, Type: "weapon_fire", AttackerSteamID: "", ExtraData: map[string]interface{}{"yaw": 0.0}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, ev := range tt.events {
				...
				gotX, hasX := ev.ExtraData["hit_x"].(float64)
				gotY, hasY := ev.ExtraData["hit_y"].(float64)
				if !expected {
					if hasX || hasY {
						t.Errorf("events[%d] should NOT have hit data, got hit_x=%v hit_y=%v", i, ev.ExtraData["hit_x"], ev.ExtraData["hit_y"])
					}
					continue
				}
				if !hasX || !hasY {
