...
func TestConvertTicks(t *testing.T) {
	...
	tests := []struct {
		name  string
		ticks []TickSnapshot
		check func(t *testing.T, got []store.InsertTickDataParams)
	}{
		{
			name: "single tick all fields",
			ticks: []TickSnapshot{
				{
					Tick: 64, SteamID: "76561198012345",
					X: 1234.5678, Y: -987.6543, Z: 128.256, Yaw: 45.5,
					Health: 100, Armor: 50, IsAlive: true, Weapon: "ak47",
				},
			},
			check: func(t *testing.T, got []store.InsertTickDataParams) {
				if len(got) != 1 {
		...
		{
			name:  "empty weapon -> NullString invalid",
			ticks: []TickSnapshot{{Tick: 0, SteamID: "123", Weapon: ""}},
			check: func(t *testing.T, got []store.InsertTickDataParams) {
		...
		{
			name:  "non-empty weapon -> NullString valid",
			ticks: []TickSnapshot{{Tick: 0, SteamID: "123", Weapon: "awp"}},
			check: func(t *testing.T, got []store.InsertTickDataParams) {
		...
		{
			name: "multiple ticks correct times and demoID",
			ticks: []TickSnapshot{
				{Tick: 0, SteamID: "A"},
				{Tick: 64, SteamID: "B"},
				{Tick: 128, SteamID: "C"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := convertTicks(tt.ticks, demoID, base, 64)
			tt.check(t, got)
		})
	}
}
