...
## 2026-05-12 — Timeline contact-moments Phase 5 (calibration harness, active-marker highlight, e2e spec)

Calibration harness in `internal/demo/contacts/detectors/calibration_test.go` (`//go:build calibration`) runs `parser.Parse → IngestRounds/Events/Visibility → contacts.Run/Persist → detectors.Run` over a 20–30 demo corpus and emits `docs/knowledge/contact-mistake-severity-calibration.md` with per-kind histograms; default `go test ./...` reports `[no tests to run]` (matches `cmd/spike-spotted` `//go:build spike` pattern). Rationale blocks (`<!-- BEGIN RATIONALE: kind -->`) survive across runs via an anchored regex pass. Severity tune: only the one decision baked into the plan landed — `shot_while_moving` 2→1 (medium → low) so the per-shot findings don't drown the tooltip's top-3; the other nine v1 kinds keep Phase 3 defaults pending the operator's corpus run. Frontend: new `findActiveContact(contacts, currentTick): ContactMarker | null` in `lib/timeline/contacts.ts` (pure, ~20 LOC); `round-timeline.tsx` derives `activeContactId` via `useMemo([contacts, currentTick])` so the marker re-renders only at window boundary crossings; `contacts-lane.tsx` paints `data-active="true"` + `ring-2 ring-amber-300 ring-offset-1 animate-pulse` on the active marker. Plan said "Show less" — actual tooltip has no collapse, only "+N more" → expand; tests adapted to that. Tooltip ordering is `severity DESC` only, not the plan's `phase ASC, severity DESC, tick ASC`; tests adapted. Playwright spec `e2e/tests/timeline-contact-moments.spec.ts` is state-based (no screenshots): probes Wails bindings + imports the reference demo via `ImportDemoByPath`, drives player selection through `__useViewerStore.setSelectedPlayer`, reads `__queryClient` for cached contact moments, asserts `isPlaying`/`currentTick`/`data-active` post-click and post-scrub. Both globals are DEV-only via `(import.meta as unknown as { env?: { DEV?: boolean } }).env?.DEV` (frontend lacks `vite-env.d.ts`). Added `data-kind={event.kind}` to `event-marker.tsx` so the round-mode spec can filter by kind without renaming the existing `event-marker-${id}` testid. LoS comment at `aim_extras.go:49` swapped: the old "we don't have line-of-sight from the parsed demo" four-line block became a six-line pointer at the `player_visibility` table and the contact pipeline. Two remaining LoS references (`micro.go`, `inputs.go`) describe geometric LoS to a target — legitimate, left in place. Two new wiki pages: `contact-moments.md` (v1 entity doc with the post-tune severity table) and `contact-mistake-severity-calibration.md` (placeholder; rationale blocks unfilled until the corpus run). `.gitignore` adds `testdata/corpus`. Parent plan README marks Phase 5 done. **Gotcha:** the format hook regenerated `wailsjs/go/models.ts` mid-session and dropped the hand-added `export type ContactOutcome = string;` alias, breaking typecheck in six files; `git checkout HEAD -- wailsjs/...` restored.

Refs: [[knowledge/contact-moments]] (new), [[knowledge/contact-mistake-severity-calibration]] (new placeholder), [[knowledge/wails-bindings]] (regen-drops-alias gotcha), [[knowledge/testing]] (build-tagged calibration-test pattern). No new ADR — ADR candidate is "derive active-marker from currentTick" but plan §04 §2 already documents the trade-off inline.

## 2026-05-12 — Timeline contact-moments Phase 4 (Wails bindings + lane swap)

Two new Wails bindings on `app.go`: `GetContactMoments(demoID, roundNumber, subjectSteam)` walks `ListContactsByDemoRoundSubject` + per-contact `ListContactMistakesByContact` and returns the parent rows with `Mistakes` embedded (single round-trip, no N+1 from the frontend); `GetRoundImportantMoments(demoID, roundNumber)` wraps `GetGameEventsByTypes` with a hardcoded `roundImportantEventTypes` set (grenade + bomb lifecycle only). Helpers: `storeContactToBinding` (strict `enemies_json` decode, lenient `extras_json` matching `GetMistakeTimeline`) and `storeContactMistakeToBinding` (`sql.NullInt64` tick → `*int32`). Frontend: new `useContactMoments` hook (`["contact-moments", demoId, roundNumber, steamId]`, `staleTime: Infinity`, gated on three inputs); `useRoundTimelineModel` swapped from `useMistakeTimeline` → `useContactMoments`; cache invalidation added in `use-recompute-analysis.ts` (the plan named `use-parse-progress.ts` but that file only mutates the demo store in this codebase). `buildLanes` input renamed `mistakes` → `contacts: main.ContactMoment[]`; step-(7) projects one `ContactMarker` per contact at `t_first` with `worstSeverity` helper; round-mode `eventsForLane` now drops `kill` / `player_hurt` / `player_flashed` so the team-mode lane shows grenades + bomb only. New `ContactTooltip` (phase-grouped, top-3 by severity DESC, `+N more` expand, ten v1 detector titles, eight outcome labels); new `ContactsLane` (severity-keyed pills incl. clean-grey 0, no-player vs no-contacts states distinct, click `pause()` → `setTick(c.tPre)` ordering). `mistakes-lane.tsx` deleted; `round-timeline.tsx` swapped to `<ContactsLane>`; `<DuelsLane>` keeps `useMistakeTimeline` (different payload, coexist by design). 45 tests added/updated across `build-lanes.test.ts`, `contact-tooltip.test.tsx`, `contacts-lane.test.tsx`, `use-contact-moments.test.ts` — all green. `wails generate module` errored silently (exit 1, no stderr) so `App.d.ts` / `App.js` / `models.ts` were hand-rolled — including a manual `export type ContactOutcome = string;` since Wails doesn't emit TS types for Go string aliases. `renderHookWithProviders` added to `src/test/render.tsx`.

Refs: [[knowledge/wails-bindings]] (codegen-skip path expanded, ContactOutcome string-alias note), [[knowledge/testing]] (`renderHookWithProviders` + `<TooltipProvider>` wrapping pattern).

## 2026-05-12 — Timeline contact-moments Phase 3 (mistake detectors)

New `internal/demo/contacts/detectors/` package: 10 v1 detectors (5 pre, 3 during, 2 post) per analysis §4.1–4.3, each ≤80 LOC, registered via a single static `Catalog` with v2 placeholders for the cross-round / heuristic rules. `runner.go` reads the post-`contacts.Persist` in-memory contact list and threads `PreviousContactEnd` per (subject, t_first ASC) so pre-window detectors clamp lookback to `max(c.TPre, prev.TLast+1)` (analysis §9.3 overlap rule). Four kinds (`slow_reaction`, `missed_first_shot`, `isolated_peek`, `shot_while_moving`) carry `WriteAggregate: true` and emit an extra `analysis.Mistake` value that the runner merges into the analyzer mistake slice **before** `analysis.PersistWithRoundMap` — required pipeline reorder in `app.go` (Persist moved after `contacts.Persist`/`detectors.Run`). One new sqlc query: `MaxDetectorVersionForDemo :one` (returns `interface{}`, same as `MaxBuilderVersionForDemo`); `coerceInt64` helper handles the assert. Catalog `Registered` slice populated in `init()` to break the static cycle `Registered → DetectorFunc → NewContactMistake → lookupCatalogEntry → Registered`. `ContactMistake` defined locally in the detectors package — root `types.go` is `package main` and isn't importable. Exported shims added: `analysis.NearestTick`/`PreviousTickRows`/`TickRangeRows`/`NearestEnemyTick`/`TeamsByRoundFromRosters` and `contacts.PartitionEventsByRound`/`PartitionVisibilityByRound`/`BuildEnemyTeam`. 14 test files: catalog invariants, weapon catalog, context-builder, 10 per-detector table tests with in-memory `AnalysisTick` fixtures, runner orchestration (empty/overlap-clamp/aggregate-writeback split), persist (idempotent roundtrip + version-gate skip + force rebuild). All pass under `go test -race`. Phase 2 scenario goldens not extended — the existing fixtures don't carry `AnalysisTicks` so most detectors can't trip on them; per-detector unit tests cover the equivalent ground.

Refs: [[knowledge/demo-parser]] (Phase 3 section), [[knowledge/sqlc-workflow]] (COALESCE → interface{} pattern), [[knowledge/testing]] (init-cycle workaround for cataloged funcs).
