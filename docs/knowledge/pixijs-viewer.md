...
- **Weapon-textures cache leak on remount.** `frontend/src/lib/pixi/sprites/weapon-textures.ts` keeps a module-level `Map<weapon, Texture>`. When the viewer is destroyed with `app.destroy({ texture: true })`, the cached `Texture` objects are destroyed but the `Map` still holds references. On remount, `getWeaponTexture` returns a stale destroyed `Texture` before re-fetching. Call `_resetWeaponTextureCache()` from `ViewerApp.destroy()` **before** `app.destroy(...)`.
- **Single-owner `TickBuffer`.** `frontend/src/stores/tick-buffer.ts` holds the active buffer; `viewer-canvas.tsx` creates it on `demoId` change, publishes via `setBuffer`, disposes + clears on unmount. Consumers like `useLoadoutSnapshot` poll the shared instance — guarded by `demoId` match so a stale tick during demo swap can't read the old buffer. Don't instantiate a second `TickBuffer` for the same `demoId`.
- **Reuse mutable scratch buffers in per-tick code.** `worldToPixelInto(out, x, y)` writes into a caller-owned `{x, y}`. `PlayerLayer` keeps a per-instance `nextById` Map and `activeSteamIds` Set, clearing instead of reallocating each tick. `TickBuffer.getFramePair` returns a singleton `FramePair` with two reusable `SampleFrame` slots — *the caller must consume the result before the next call*, documented in a contract comment in `tick-buffer.ts`.
- **Unload previous map texture on switch.** `MapLayer` tracks `_loadedUrl` and fires `Assets.unload(previousUrl)` when switching radars. Without it, ~4 MB GPU per previous radar accumulates across heavy session map-hopping.
- **Drop the gameEvents query cache after `EventLayer` consumes it.** `viewer-canvas.tsx` tracks `consumedDemoId`; once `EventLayer.setEvents` runs, the observer flips off and `queryClient.removeQueries({queryKey: ["game-events", demoId], exact: true})` evicts the cache. Without this, the events payload is retained 3× (TanStack cache + ref + EventLayer's internal copy).
